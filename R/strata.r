strata<-function(data, stratanames=NULL, size, method=c("srswor","srswr","poisson","systematic"),pik,description=FALSE)
{
  ...
  else
  {
    ...
    index=1:nrow(data)
    original_stratanames <- stratanames
    if(stratanames != make.names(stratanames)){
      stratanames <- make.names(stratanames)
    ...
    if(method=="srswr")
      colnames(result)=c(original_stratanames,"ID_unit","Prob","Stratum")
    else
      colnames(result)=c(original_stratanames,"ID_unit","Prob","Stratum")
